...
class SearchCache:
    """
    Search tool cache class. Uses SQLite to store cache data and supports fast indexed lookups.
    
    Data model:
    - search_cache table: stores query -> result (query is the primary key)
    
    Features:
    - Uses SQLite with indexing for fast queries
    - Quickly locates search results via the query primary key
    ...
    Usage example:
        ...
        cache = SearchCache(resume=False)
        
        # Quickly fetch search results by query
        result = cache.get("python tutorial")
        
        # Write to cache
        cache.set("python tutorial", "search results...")
    """
        ...
        if resume and db_exists:
            ...
            try:
                ...
                if cursor.fetchone() is None:
                    # Table missing; initialization required
    ...
    def _init_database(self):
        ...
        with self._lock:
            ...
            # Create the search_cache table: store query -> result
            # query is the primary key because search results depend only on query
            cursor.execute("""
            ...
            # Note: the PRIMARY KEY constraint automatically creates an index
            self._conn.commit()
    ...
    def get(self, query: str) -> Optional[str]:
        ...
        Returns:
            search result, or None if it does not exist
        """
    ...
    def set(self, query: str, result: str):
        """
        Write search results to the cache (uses INSERT OR REPLACE to support updates)
        ...
        try:
            ...
            # Insert or update the search_cache table (query is the primary key)
            self._execute_write("""
...
class Search(BaseTool):
    ...
    def __init__(self, cfg: Optional[dict] = None):
        """Initialize the Search tool and create the cache instance"""
        super().__init__(cfg)
    ...
    def search_with_serp(self, query: str):
        # Cache lookup logic
        if self.cache:
            # Check the cache first; return immediately on a hit
            cached_result = self.cache.get(query)
            if cached_result:
                print(f"[search] Cache hit for query: {query}")
                return cached_result
        
        # If not found in cache, perform the search
        result = self.google_search_with_serp(query)
        
        # Cache only successful search results (normal status code)
        # Do not cache failures (for example, "No results found" or "Google search Timeout")
