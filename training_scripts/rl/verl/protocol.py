...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        def _safe_repr(value: Any, max_chars: int = 1200) -> str:
            try:
                text = repr(value)
            except Exception as err:  # pragma: no cover
                text = f"<repr_failed: {type(err).__name__}: {err}>"
            if len(text) > max_chars:
                return text[:max_chars] + "...(truncated)"
            return text

        # Collect non_tensor_batch with heterogeneous keys across workers.
        # Workers may have different keys (e.g. reward_extra_info varies by
        # eval script).  Use the union of all keys; fill missing entries with
        # a None-array of the corresponding batch size.
        all_nt_keys: set[str] = set()
        for d in data:
        ...
        for key in all_nt_keys:
            arrays = []
            for d in data:
                if d.non_tensor_batch and key in d.non_tensor_batch:
                    arrays.append(d.non_tensor_batch[key])
                else:
                    # Determine the batch size of this DataProto for padding
                    d_bsz = d.batch.batch_size[0] if d.batch is not None else 0
                    placeholder = np.empty(d_bsz, dtype=object)
                    placeholder[:] = None
            ...
            try:
                ...
                debug_lines = [
                    f"[DataProto.concat] Failed to concatenate non_tensor key={key!r}.",
                    f"original_error={type(err).__name__}: {err}",
                    f"num_arrays={len(arrays)}",
                ]
                for idx, arr in enumerate(arrays):
                    if isinstance(arr, np.ndarray):
                        ...
                        arr_dtype = arr.dtype
                        arr_size = arr.size
                        head_count = min(3, arr_shape[0] if arr_ndim > 0 else 1)
                        if arr_ndim > 0 and arr_size > 0:
                            arr_head = _safe_repr(arr[:head_count])
                        elif arr_size > 0:
                            arr_head = _safe_repr(arr)
                        else:
                            arr_head = "<empty>"
                    else:
                        arr_shape = None
                        ...
                        arr_head = _safe_repr(arr)
                    debug_lines.append(
                        f"  - arrays[{idx}]: type={type(arr).__name__}, "
                        f"shape={arr_shape}, ndim={arr_ndim}, dtype={arr_dtype}, size={arr_size}, "
                        f"preview={arr_head}"
                    )
                debug_msg = "\n".join(debug_lines)
                print(debug_msg, flush=True)
                raise ValueError(debug_msg) from err
            ...
            for d in data:
                for k, v in d.meta_info.items():
                    if k == "metrics":
                        ...
                        # Different workers can observe different key orders; normalize to a stable union.
                        if v is None:
                            continue
                        incoming = list(v)
                        if k in merged_meta_info and merged_meta_info[k] is not None:
                            merged_meta_info[k] = sorted(set(merged_meta_info[k]).union(incoming))
                        else:
                            merged_meta_info[k] = sorted(set(incoming))
                    else:
