...
def _compute_rollout_weighted_advantage_mean(
    advantages: torch.Tensor, response_mask: torch.Tensor, batch: DataProto
) -> float | None:
    """Average one advantage value per unique rollout instead of per valid token."""
    if advantages.size(0) == 0:
        return None

    row_has_valid_token = response_mask.any(dim=-1)
    if not row_has_valid_token.any():
        return None

    row_advantages = []
    row_keys = []
    uid_values = batch.non_tensor_batch.get("uid")
    rollout_values = batch.non_tensor_batch.get("rollout_index")

    for i in range(advantages.size(0)):
        if not bool(row_has_valid_token[i].item()):
            continue

        valid_adv = advantages[i][response_mask[i]]
        if valid_adv.numel() == 0:
            continue

        # GRPO broadcasts one rollout-level advantage to every valid token. Taking
        # the row mean is robust even if future estimators stop being constant.
        row_advantages.append(valid_adv.mean().detach())

        uid = uid_values[i] if uid_values is not None and len(uid_values) == advantages.size(0) else i
        rollout_idx = rollout_values[i] if rollout_values is not None and len(rollout_values) == advantages.size(0) else i
        row_keys.append((uid, rollout_idx))

    if not row_advantages:
        return None

    rollout_to_advantages = defaultdict(list)
    for key, adv in zip(row_keys, row_advantages, strict=False):
        rollout_to_advantages[key].append(adv)

    unique_rollout_advantages = [torch.stack(adv_list).mean() for adv_list in rollout_to_advantages.values()]
    return torch.stack(unique_rollout_advantages).mean().item()
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    token_weighted_adv_mean = torch.mean(valid_adv).detach().item()
    rollout_weighted_adv_mean = _compute_rollout_weighted_advantage_mean(advantages, response_mask, batch)
    ...
    metrics = {
        ...
        # adv
        "critic/advantages/mean": token_weighted_adv_mean,
        "critic/advantages/mean_token_weighted": token_weighted_adv_mean,
        "critic/advantages/mean_rollout_weighted": (
            rollout_weighted_adv_mean if rollout_weighted_adv_mean is not None else token_weighted_adv_mean
        ),
