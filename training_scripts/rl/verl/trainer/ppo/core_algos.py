...
def compute_grpo_outcome_advantage(
    ...
    response_mask: torch.Tensor,
    index: np.ndarray,
    rollout_index: Optional[np.ndarray] = None,
    epsilon: float = 1e-6,
    ...
    Args:
        ...
        rollout_index: `(Optional[np.ndarray])`
            Optional rollout-level index. When provided, rows with the same
            `(index, rollout_index)` are treated as one original rollout for
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        use_rollout_dedup = False
        if rollout_index is not None:
            try:
                use_rollout_dedup = len(rollout_index) == bsz
            except TypeError:
                use_rollout_dedup = False

        if use_rollout_dedup:
            # Keep only one statistic contribution per original rollout.
            rollout2rows = defaultdict(list)
            rollout2prompt = {}
            for i in range(bsz):
                prompt_idx = _to_py_scalar(index[i])
                rollout_idx = _to_py_scalar(rollout_index[i])
                rollout_key = (prompt_idx, rollout_idx)
                rollout2rows[rollout_key].append(i)
                rollout2prompt[rollout_key] = prompt_idx

            rollout2score = {}
            for rollout_key, rows in rollout2rows.items():
                if len(rows) == 1:
                    rollout_score = scores[rows[0]]
                else:
                    rollout_score = scores[rows].mean()
                rollout2score[rollout_key] = rollout_score
                id2score[rollout2prompt[rollout_key]].append(rollout_score)
            ...
            row_scores = torch.empty_like(scores)
            for i in range(bsz):
                prompt_idx = _to_py_scalar(index[i])
                rollout_idx = _to_py_scalar(rollout_index[i])
                rollout_key = (prompt_idx, rollout_idx)
                rollout_score = rollout2score[rollout_key]
                if norm_adv_by_std_in_grpo:
                    row_scores[i] = (rollout_score - id2mean[prompt_idx]) / (id2std[prompt_idx] + epsilon)
                else:
                    row_scores[i] = rollout_score - id2mean[prompt_idx]
            row_scores = row_scores.unsqueeze(-1) * response_mask
            return row_scores, row_scores

        for i in range(bsz):
            id2score[_to_py_scalar(index[i])].append(scores[i])
        for idx in id2score:
            if len(id2score[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0, device=scores.device, dtype=scores.dtype)
                id2std[idx] = torch.tensor(1.0, device=scores.device, dtype=scores.dtype)
...
def compute_grpo_vectorized_outcome_advantage(
    ...
    """
    if rollout_index is not None:
        # Preserve "one statistic per original rollout" semantics when session-expanded
        # samples are present.
...
def agg_loss(
    ...
    session_row_weight: Optional[torch.Tensor] = None,
):
    """
    ...
    Args:
        ...
        session_mask: optional mask indicating session end positions, (bs, response_length)
            Used for "session-mean-token-sum" mode. 1 at end of each session, 0 otherwise.
        session_row_weight: optional per-row weight for rollout-equivalent weighting, (bs,).
            When provided, each row's seq_loss is scaled by this weight so that split sessions
            collectively behave like one unsplit trajectory under token-mean.
            Computed as row_assistant_tokens / rollout_total_assistant_tokens.
    ...
    elif loss_agg_mode == "seq-mean-token-mean": # GSPO
        seq_mask = torch.sum(loss_mask, dim=-1)  # per-sequence token count
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1) / (seq_mask + 1e-8)  # token-mean
        if session_row_weight is not None:
            # session_row_weight is pre-scaled at expansion time so that sum(weights) = B (expanded
            # batch size) instead of R (unique rollouts).  This makes the existing global_batch_size
            # denominator DP-correct without needing all-reduce or a new parameter.
            seq_losses = seq_losses * session_row_weight
    ...
    elif loss_agg_mode == "session-mean-token-sum":
        # Session-level loss aggregation for multi-turn with condenser
        # Each session's tokens are summed, then averaged across sessions
        if session_mask is None:
            # Fallback to seq-mean if no session_mask provided
            seq_losses = torch.sum(loss_mat * loss_mask, dim=-1)
            ...
            # Sum loss for each session, then average across sessions
            batch_size, response_length = loss_mat.shape
            masked_loss = loss_mat * loss_mask

            # Compute cumulative sum of losses
            cumsum_loss = masked_loss.cumsum(dim=-1)

            # Get loss at session boundaries
            session_end_cumsum = cumsum_loss * session_mask

            # Compute per-session loss by subtracting previous session cumsum
            # For efficiency, we compute this differently:
            # Total session loss = sum at session end - sum at previous session end
            session_losses = torch.zeros_like(cumsum_loss)
            for i in range(batch_size):
                session_ends = session_mask[i].nonzero(as_tuple=True)[0]
                if len(session_ends) > 0:
                    prev_cumsum = 0.0
                    for j, end in enumerate(session_ends):
                        session_losses[i, end] = cumsum_loss[i, end] - prev_cumsum
                        prev_cumsum = cumsum_loss[i, end]

            # Sum all session losses and count sessions
            total_session_loss = (session_losses * session_mask).sum()
            num_sessions = session_mask.sum()
            if num_sessions == 0:
                num_sessions = 1

            loss = total_session_loss / num_sessions * dp_size
    else:
...
def compute_policy_loss_vanilla(
    ...
    # Extract session_mask and session_row_weight from global_batch_info if present
    global_batch_info = dict(config.global_batch_info) if config.global_batch_info else {}
    session_mask = global_batch_info.pop("session_mask", None)
    session_row_weight = global_batch_info.pop("session_row_weight", None)

    pg_loss = agg_loss(
        loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode,
        session_mask=session_mask, session_row_weight=session_row_weight, **global_batch_info
    )
...
def compute_value_loss(
    ...
    vf_loss = 0.5 * agg_loss(loss_mat=clipped_vf_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode, session_row_weight=session_row_weight)
