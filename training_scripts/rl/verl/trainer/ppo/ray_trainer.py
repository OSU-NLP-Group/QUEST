...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            ...
            rollout_index=data.non_tensor_batch.get("rollout_index", None),
        ...
        if adv_estimator == AdvantageEstimator.GRPO_VECTORIZED and "rollout_index" in data.non_tensor_batch:
            adv_kwargs["rollout_index"] = data.non_tensor_batch["rollout_index"]
...
class RayPPOTrainer:
    ...
    def _on_val_batch_complete(self, batch, inputs, outputs, scores):
        """Hook called after each validation batch is scored. Override for custom logging."""
        pass
    ...
    def _compute_old_log_prob(self, batch: DataProto):
        if self.use_legacy_worker_impl == "disable":
            ...
            batch_td = left_right_2_no_padding(batch_td)
            # step 3: add meta info
            # Entropy is only needed when the PPO loss actually uses it. Avoid
            # materializing entropy tensors during old-log-prob recomputation
            # when entropy regularization is disabled, which saves memory in the
            # already memory-bound compute_log_prob path.
    ...
    def _update_actor(self, batch: DataProto) -> DataProto:
        ...
        # Filter out samples with empty response (response_mask all zeros) to avoid
        # downstream RoPE / seqlen errors when a session produced no valid tokens.
        if "response_mask" in batch.batch:
            valid_mask = batch.batch["response_mask"].sum(dim=-1) > 0
            n_dropped = int((~valid_mask).sum())
            if n_dropped > 0:
                import logging as _logging
                _logging.getLogger(__name__).warning(
                    f"_update_actor: dropping {n_dropped} samples with empty response_mask"
                )
                batch = batch[valid_mask.nonzero(as_tuple=False).squeeze(1)]
        if len(batch) == 0:
            return DataProto.from_single_dict(
                data={},
                meta_info={"metrics": {"actor/empty_batch_skipped": 1.0, "perf/mfu/actor": 0.0}},
            )
        dp_size = self._get_dp_size(self.actor_rollout_wg, "actor")
        batch_size_before_dp_pad = len(batch)
        batch.batch["__is_pad__"] = torch.zeros(batch_size_before_dp_pad, dtype=torch.bool)
        if batch_size_before_dp_pad % dp_size != 0:
            import logging as _logging
            ...
            batch, dp_pad_size = pad_dataproto_to_divisor(batch_for_pad, dp_size)
            batch.meta_info.update(original_meta_info)
            batch_size_after_dp_pad = len(batch)
            _logging.getLogger(__name__).warning(
                "_update_actor: padded batch for actor dispatch divisibility: "
                "original=%s, dp_size=%s, pad_size=%s, padded=%s",
                batch_size_before_dp_pad,
                dp_size,
                dp_pad_size,
                batch_size_after_dp_pad,
            )
