...
class MegatronCheckpointManager(BaseCheckpointManager):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        log_with_rank(
            "[CKPT_MANAGER_VERSION=MOE_LOCAL_OPTIMIZER_EXTRA_V1] "
            f"file={__file__} role={self.role} use_dist_checkpointing={self.use_dist_checkpointing} "
            f"use_hf_checkpoint={self.use_hf_checkpoint} num_experts={getattr(self.hf_config, 'num_experts', 'NA')} "
            f"local_optimizer_extra_fallback={self._should_use_local_optimizer_extra_checkpoint()}",
            rank=self.rank,
            logger=logger,
        )

    def _is_moe_model(self) -> bool:
        return getattr(self.hf_config, "num_experts", 1) > 1

    def _should_use_local_optimizer_extra_checkpoint(self) -> bool:
        # MoE optimizer/extra state save can hang in Megatron dist_checkpointing.
        return self._is_moe_model()

    def _get_local_optimizer_extra_checkpoint_path(self, local_path: str) -> str:
        return self.get_checkpoint_name(local_path, return_base_dir=False, basename="optim_rng.pt")

    def _should_save_local_optimizer_parameter_state(self) -> bool:
        return (
            self._should_use_local_optimizer_extra_checkpoint()
            and self.use_distributed_optimizer
            and bool(getattr(self.checkpoint_config, "save_distributed_optimizer_parameter_state", False))
        )

    def _get_local_optimizer_parameter_state_checkpoint_path(self, local_path: str) -> str:
        return self.get_checkpoint_name(local_path, return_base_dir=False, basename="optim_param_state.pt")

    def _cleanup_existing_hf_weight_files(self, hf_ckpt_path: str) -> None:
        ...
        for entry in os.listdir(hf_ckpt_path):
            if entry.endswith(".safetensors") or entry.endswith(".safetensors.index.json"):
                file_path = os.path.join(hf_ckpt_path, entry)
                if os.path.isfile(file_path):
                    try:
                        os.remove(file_path)
                        removed_files.append(entry)
                    except FileNotFoundError:
                        # Another rank may have removed the same stale shard first.
                        continue

        if removed_files:
            log_with_rank(
                "[SAVE_STAGE=HF_CKPT_CLEANUP][MegatronCheckpointManager.save_checkpoint] "
                f"removed stale HF weight files from {hf_ckpt_path}: {sorted(removed_files)}",
                rank=self.rank,
                logger=logger,
            )
    ...
    def generate_state_dict(
        ...
        log_with_rank(
            "[SAVE_STAGE=GENERATE_MODEL_STATE_BEGIN][MegatronCheckpointManager.generate_state_dict] "
            f"generate_model={generate_model} generate_optimizer={generate_optimizer} "
            f"generate_extra={generate_extra} is_loading={is_loading}",
            rank=self.rank,
            logger=logger,
        )
        for vpp_rank, model in enumerate(self.model):
            ...
            log_with_rank(
                "[SAVE_STAGE=GENERATE_MODEL_STATE_CHUNK_BEGIN][MegatronCheckpointManager.generate_state_dict] "
                f"key={key}",
                rank=self.rank,
                logger=logger,
            )
            ...
            log_with_rank(
                "[SAVE_STAGE=GENERATE_MODEL_STATE_CHUNK_END][MegatronCheckpointManager.generate_state_dict] "
                f"key={key}",
                rank=self.rank,
                logger=logger,
            )
        log_with_rank(
            "[SAVE_STAGE=GENERATE_MODEL_STATE_END][MegatronCheckpointManager.generate_state_dict] "
            f"keys={[k for k in state_dict.keys() if str(k).startswith('model')]}",
            rank=self.rank,
            logger=logger,
        )
        ...
        if generate_optimizer:
            log_with_rank(
                "[SAVE_STAGE=GENERATE_OPTIMIZER_STATE_BEGIN][MegatronCheckpointManager.generate_state_dict]",
                rank=self.rank,
                logger=logger,
            )
            log_with_rank(
                "[SAVE_STAGE=GENERATE_OPTIMIZER_STATE_BEFORE_BARRIER][MegatronCheckpointManager.generate_state_dict]",
                rank=self.rank,
                logger=logger,
            )
            torch.distributed.barrier()
            log_with_rank(
                "[SAVE_STAGE=GENERATE_OPTIMIZER_STATE_AFTER_BARRIER][MegatronCheckpointManager.generate_state_dict]",
                rank=self.rank,
                logger=logger,
            )
            ...
            log_with_rank(
                "[SAVE_STAGE=GENERATE_OPTIMIZER_STATE_BEFORE_SHARDED_STATE_DICT]"
                "[MegatronCheckpointManager.generate_state_dict]",
                rank=self.rank,
                logger=logger,
            )
            ...
            log_with_rank(
                "[SAVE_STAGE=GENERATE_OPTIMIZER_STATE_AFTER_SHARDED_STATE_DICT]"
                "[MegatronCheckpointManager.generate_state_dict]",
                rank=self.rank,
                logger=logger,
            )
            ...
            if self.lr_scheduler is not None:
                log_with_rank(
                    "[SAVE_STAGE=GENERATE_LR_SCHEDULER_STATE_BEGIN][MegatronCheckpointManager.generate_state_dict]",
                    rank=self.rank,
                    logger=logger,
                )
                ...
                log_with_rank(
                    "[SAVE_STAGE=GENERATE_LR_SCHEDULER_STATE_END][MegatronCheckpointManager.generate_state_dict]",
                    rank=self.rank,
                    logger=logger,
                )
            log_with_rank(
                "[SAVE_STAGE=GENERATE_OPTIMIZER_STATE_END][MegatronCheckpointManager.generate_state_dict]",
                rank=self.rank,
                logger=logger,
            )
        ...
        if generate_extra:
            log_with_rank(
                "[SAVE_STAGE=GENERATE_EXTRA_STATE_BEGIN][MegatronCheckpointManager.generate_state_dict]",
                rank=self.rank,
                logger=logger,
            )
            log_with_rank(
                "[SAVE_STAGE=GENERATE_EXTRA_STATE_BEFORE_BARRIER][MegatronCheckpointManager.generate_state_dict]",
                rank=self.rank,
                logger=logger,
            )
            torch.distributed.barrier()
            log_with_rank(
                "[SAVE_STAGE=GENERATE_EXTRA_STATE_AFTER_BARRIER][MegatronCheckpointManager.generate_state_dict]",
                rank=self.rank,
                logger=logger,
            )
            ...
            log_with_rank(
                "[SAVE_STAGE=GENERATE_EXTRA_STATE_END][MegatronCheckpointManager.generate_state_dict]",
                rank=self.rank,
                logger=logger,
            )
    ...
    def generate_local_optimizer_extra_state_dict(self, metadata: dict | None = None):
        state_dict = {}
        ...
        if self.should_save_extra:
            state_dict["rng_state"] = self.get_rng_state(use_dist_ckpt=False)
        return state_dict
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        local_optimizer_extra_path = self._get_local_optimizer_extra_checkpoint_path(local_path)
        local_optimizer_param_state_path = self._get_local_optimizer_parameter_state_checkpoint_path(local_path)
        local_optimizer_extra_state = None
        if os.path.exists(local_optimizer_extra_path):
            local_optimizer_extra_state = torch.load(local_optimizer_extra_path, map_location="cpu", weights_only=False)
            log_with_rank(
                f"Loaded local optimizer/extra checkpoint from {local_optimizer_extra_path}",
                rank=self.rank,
                logger=logger,
            )

        need_dist_checkpoint = (
            (self.should_load_model and self.use_dist_checkpointing)
            or (self.should_load_optimizer and local_optimizer_extra_state is None)
            or (self.should_load_extra and local_optimizer_extra_state is None)
        )
        sharded_sd_metadata = None
        if need_dist_checkpoint:
        ...
        sharded_state_dict = self.generate_state_dict(
            self.should_load_model and self.use_dist_checkpointing,
            self.should_load_optimizer and local_optimizer_extra_state is None,
            self.should_load_extra and local_optimizer_extra_state is None,
            is_loading=True,
        ...
        if self.should_load_optimizer:
            optimizer_source_state = local_optimizer_extra_state or state_dict
            assert "optimizer" in optimizer_source_state, (
                f"Optimizer state dict not found in {optimizer_source_state.keys()}. Please check the checkpoint file {local_path}."
            )
            optimizer_state_dict = optimizer_source_state["optimizer"]
            self.optimizer.load_state_dict(optimizer_state_dict)
            log_with_rank(f"Loaded optimizer checkpoint from {local_path}", rank=self.rank, logger=logger)
            if self._should_save_local_optimizer_parameter_state():
                if os.path.exists(local_optimizer_param_state_path):
                    log_with_rank(
                        "[LOAD_STAGE=LOCAL_OPT_PARAM_STATE_BEGIN][MegatronCheckpointManager.load_checkpoint] "
                        f"path={local_optimizer_param_state_path}",
                        rank=self.rank,
                        logger=logger,
                    )
                    self.optimizer.load_parameter_state(local_optimizer_param_state_path)
                    torch.distributed.barrier()
                    log_with_rank(
                        "[LOAD_STAGE=LOCAL_OPT_PARAM_STATE_END][MegatronCheckpointManager.load_checkpoint] "
                        f"path={local_optimizer_param_state_path}",
                        rank=self.rank,
                        logger=logger,
                    )
                    ...
                    log_with_rank(
                        "[LOAD_STAGE=LOCAL_OPT_PARAM_STATE_MISSING][MegatronCheckpointManager.load_checkpoint] "
                        f"path={local_optimizer_param_state_path} missing; optimizer moments will not be restored",
                        rank=self.rank,
                        logger=logger,
                    )
            if self.use_checkpoint_opt_param_scheduler:
                assert "lr_scheduler" in optimizer_source_state, (
                    f"LR scheduler state dict not found in {optimizer_source_state.keys()}. Please check the checkpoint file "
                    f"{local_path}."
                )
                lr_scheduler_state_dict = optimizer_source_state["lr_scheduler"]
                if self.lr_scheduler is not None:
                    self.lr_scheduler.load_state_dict(lr_scheduler_state_dict)
                    log_with_rank(f"Loaded LR scheduler checkpoint from {local_path}", rank=self.rank, logger=logger)

        if self.should_load_extra:
            extra_source_state = local_optimizer_extra_state or state_dict
            assert "rng_state" in extra_source_state, (
                f"RNG state dict not found in {extra_source_state.keys()}. Please check the checkpoint file {local_path}."
            )
            rng_state = extra_source_state["rng_state"]
            self.load_rng_states(rng_state, use_dist_ckpt=local_optimizer_extra_state is None)
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        dist_checkpoint_path = get_dist_checkpoint_path(local_path)
        use_local_optimizer_extra_checkpoint = self._should_use_local_optimizer_extra_checkpoint()
        log_with_rank(
            "[SAVE_STAGE=CKPT_ENTER][MegatronCheckpointManager.save_checkpoint] "
            f"enter local_path={local_path} dist_path={dist_checkpoint_path} global_step={global_step} "
            f"use_dist_checkpointing={self.use_dist_checkpointing} async_save={self.checkpoint_config.async_save} "
            f"save_model={self.should_save_model} save_optimizer={self.should_save_optimizer} "
            f"save_extra={self.should_save_extra}",
            rank=self.rank,
            logger=logger,
        )
        ...
        if self.use_dist_checkpointing:
            ...
            state_dict = self.generate_state_dict(
                ...
                self.should_save_optimizer and not use_local_optimizer_extra_checkpoint,
                self.should_save_extra and not use_local_optimizer_extra_checkpoint,
                metadata=sharded_sd_metadata,
            )
            ...
            log_with_rank(
                "[SAVE_STAGE=CKPT_BEFORE_DIST_SAVE][MegatronCheckpointManager.save_checkpoint] "
                f"before save_dist_checkpointing dist_path={dist_checkpoint_path} "
                f"state_dict_keys={list(state_dict.keys())}",
                rank=self.rank,
                logger=logger,
            )
            ...
            log_with_rank(
                "[SAVE_STAGE=CKPT_AFTER_DIST_SAVE][MegatronCheckpointManager.save_checkpoint] "
                f"after save_dist_checkpointing dist_path={dist_checkpoint_path} "
                f"async_request={'set' if async_save_request is not None else 'none'}",
                rank=self.rank,
                logger=logger,
            )
            ...
            if not self.checkpoint_config.async_save:
                ...
                log_with_rank(
                    "[SAVE_STAGE=CKPT_BEFORE_INTERNAL_BARRIER][MegatronCheckpointManager.save_checkpoint] "
                    f"before internal barrier dist_path={dist_checkpoint_path}",
                    rank=self.rank,
                    logger=logger,
                )
                torch.distributed.barrier()
                log_with_rank(
                    "[SAVE_STAGE=CKPT_AFTER_INTERNAL_BARRIER][MegatronCheckpointManager.save_checkpoint] "
                    f"after internal barrier dist_path={dist_checkpoint_path}",
                    rank=self.rank,
                    logger=logger,
                )
            if use_local_optimizer_extra_checkpoint:
                local_optimizer_extra_state = self.generate_local_optimizer_extra_state_dict(metadata=sharded_sd_metadata)
                local_optimizer_extra_path = self._get_local_optimizer_extra_checkpoint_path(local_path)
                log_with_rank(
                    "[SAVE_STAGE=CKPT_BEFORE_LOCAL_TORCH_SAVE][MegatronCheckpointManager.save_checkpoint] "
                    f"path={local_optimizer_extra_path} keys={list(local_optimizer_extra_state.keys())}",
                    rank=self.rank,
                    logger=logger,
                )
                torch.save(local_optimizer_extra_state, local_optimizer_extra_path)
                log_with_rank(
                    "[SAVE_STAGE=CKPT_AFTER_LOCAL_TORCH_SAVE][MegatronCheckpointManager.save_checkpoint] "
                    f"path={local_optimizer_extra_path} keys={list(local_optimizer_extra_state.keys())}",
                    rank=self.rank,
                    logger=logger,
                )
                if self._should_save_local_optimizer_parameter_state() and self.should_save_optimizer:
                    local_optimizer_param_state_path = self._get_local_optimizer_parameter_state_checkpoint_path(local_path)
                    log_with_rank(
                        "[SAVE_STAGE=CKPT_BEFORE_LOCAL_OPT_PARAM_STATE_SAVE][MegatronCheckpointManager.save_checkpoint] "
                        f"path={local_optimizer_param_state_path}",
                        rank=self.rank,
                        logger=logger,
                    )
                    self.optimizer.save_parameter_state(local_optimizer_param_state_path)
                    torch.distributed.barrier()
                    log_with_rank(
                        "[SAVE_STAGE=CKPT_AFTER_LOCAL_OPT_PARAM_STATE_SAVE][MegatronCheckpointManager.save_checkpoint] "
                        f"path={local_optimizer_param_state_path}",
                        rank=self.rank,
                        logger=logger,
                    )
        ...
        if self.should_save_model:
            ...
            if self.peft_cls is not None:
                ...
                self._cleanup_existing_hf_weight_files(hf_ckpt_path)
                if self.vanilla_bridge:
                    extended_args = {}
