...
def rearrange_micro_batches(
    ...
    if total_seqlen == 0 or num_micro_batches <= 0:
        ...
        debug_info = {
            ...
            "total_seqlen": int(total_seqlen),
            "num_micro_batches": int(num_micro_batches),
            "num_batches_divided_by": num_batches_divided_by,
            "same_micro_num_in_dp": same_micro_num_in_dp,
            "seq_len_effective": seq_len_effective.cpu().tolist(),
        }
        logger.error("Invalid dynamic micro-batch partition input: %s", debug_info)
        raise ValueError(f"Invalid dynamic micro-batch partition input: {debug_info}")

    assert num_micro_batches <= len(seq_len_effective), (
        f"num_micro_batches={num_micro_batches} exceeds local batch size={len(seq_len_effective)}. "
        "This usually means DP ranks lost samples inconsistently before dynamic micro-batch splitting; "
        "check padding rows and empty-response filtering."
    )
