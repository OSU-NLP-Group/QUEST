...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        # Include session row weights for rollout-equivalent loss weighting
        if "session_row_weight" in data.batch.keys():
            select_keys.append("session_row_weight")
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # Clear stale session_row_weight before populating from current batch
                    self.config.global_batch_info.pop("session_row_weight", None)
                    session_row_weight_for_policy = model_inputs.get("session_row_weight", None)
                    if session_row_weight_for_policy is not None:
                        self.config.global_batch_info["session_row_weight"] = session_row_weight_for_policy
                    ...
                    # Get session_mask for session-level loss aggregation (DeepResearch)
                    session_mask = model_inputs.get("session_mask", None)
                    # Get session_row_weight for rollout-equivalent loss weighting (session weight correction)
                    session_row_weight = model_inputs.get("session_row_weight", None)

                    if calculate_entropy and entropy is not None:
                        entropy_agg = agg_loss(
                            loss_mat=entropy, loss_mask=response_mask, loss_agg_mode=loss_agg_mode,
                            session_mask=session_mask, session_row_weight=session_row_weight,
                        )
