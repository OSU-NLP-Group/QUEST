...
def ppo_loss(config: ActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    # session_mask for session-level loss aggregation (DeepResearch)
    config.global_batch_info.pop("session_mask", None)
    if "session_mask" in data.keys():
        config.global_batch_info["session_mask"] = data["session_mask"]
    # session_row_weight for rollout-equivalent loss weighting (session weight correction)
    config.global_batch_info.pop("session_row_weight", None)
    if "session_row_weight" in data.keys():
        config.global_batch_info["session_row_weight"] = data["session_row_weight"]
...
def value_loss(config: CriticConfig, model_output, data: TensorDict, dp_group=None):
    ...
    session_row_weight = data.get("session_row_weight", None)
