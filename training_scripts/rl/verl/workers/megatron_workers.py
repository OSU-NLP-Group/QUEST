...
def _patch_megatron_gate_slice():
    """Fix: Megatron SelfAttention.get_query_key_value_tensors slices `query` for the
    num_kv_heads < TP_size case but forgets to apply the same slice to `gate`.
    This causes _apply_output_gate to fail with a shape mismatch when the two tensors
    have different numbers of heads (e.g. Qwen3.5-MoE with TP=4, num_kv_heads=2).
    """
    try:
        from megatron.core.transformer.attention import SelfAttention
        from megatron.core.parallel_state import get_tensor_model_parallel_rank

        if getattr(SelfAttention.get_query_key_value_tensors, "_verl_gate_patch", False):
            return

        _orig = SelfAttention.get_query_key_value_tensors

        def _patched(self, hidden_states, key_value_states=None, output_gate=False, split_qkv=True):
            result = _orig(self, hidden_states, key_value_states, output_gate, split_qkv)
            if output_gate and isinstance(result, tuple) and len(result) == 4:
                query, key, value, gate = result
                if (
                    gate.dim() == 4
                    and gate.shape[2] != query.shape[2]
        ...
        _patched._verl_gate_patch = True
        SelfAttention.get_query_key_value_tensors = _patched
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f"_patch_megatron_gate_slice failed: {e}")


_patch_megatron_gate_slice()
...
def _count_megatron_optimizer_state_entries(optimizer) -> int:
    if optimizer is None:
        return 0
    if hasattr(optimizer, "chained_optimizers"):
        return sum(_count_megatron_optimizer_state_entries(opt) for opt in optimizer.chained_optimizers)
    inner_optimizer = getattr(optimizer, "optimizer", None)
    if inner_optimizer is None or not hasattr(inner_optimizer, "state"):
        return 0
    return len(inner_optimizer.state)


def _initialize_megatron_optimizer_state_for_debug(optimizer) -> int:
    if optimizer is None:
        return 0
    if hasattr(optimizer, "chained_optimizers"):
        return sum(_initialize_megatron_optimizer_state_for_debug(opt) for opt in optimizer.chained_optimizers)

    inner_optimizer = getattr(optimizer, "optimizer", None)
    init_state_fn = getattr(optimizer, "init_state_fn", None)
    config = getattr(optimizer, "config", None)
    if inner_optimizer is None or not callable(init_state_fn):
        return 0

    init_state_fn(inner_optimizer, config)
    ...
    for group in inner_optimizer.param_groups:
        for param in group["params"]:
            ...
            if "step" not in state:
                state["step"] = torch.tensor(0.0, dtype=torch.float32, device=state["exp_avg"].device)
            initialized += 1
    return initialized
...
class ActorRolloutRefWorker(MegatronWorker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if self._is_actor or self._is_rollout:
            ...
            if self.config.actor.load_weight:
                if self.config.actor.megatron.use_dist_checkpointing and self.config.actor.megatron.dist_checkpointing_path:
            ...
            if self.config.ref.load_weight:  # should align with the actor:
                ...
                if self.config.ref.megatron.use_dist_checkpointing and self.config.ref.megatron.dist_checkpointing_path:
                    load_mcore_dist_weights(
                        ref_module,
    ...
    def update_actor(self, data: DataProto):
        ...
        if "response_mask" in data.batch and "__is_pad__" not in data.batch:
            valid_mask = data.batch["response_mask"].sum(dim=-1) > 0
            n_dropped = int((~valid_mask).sum())
            if n_dropped > 0:
                logger.warning("update_actor: dropping %s padded/empty samples before optimization", n_dropped)
                data = data[valid_mask.nonzero(as_tuple=False).squeeze(1)]
        if len(data) == 0:
            output = DataProto(meta_info={"metrics": {"actor/empty_batch_skipped": 1.0, "perf/mfu/actor": 0.0}})
            return output.to("cpu")
        data.meta_info["micro_batch_size"] = micro_batch_size
        # Adjust ppo_mini_batch_size if the actual batch size is not divisible (e.g. after
        # empty-response filtering in _update_actor).
        actual_batch_size = data.batch.batch_size[0]
        original_mini_batch_size = self.actor.config.ppo_mini_batch_size
        if original_mini_batch_size and actual_batch_size % original_mini_batch_size != 0:
            if self.actor.config.use_dynamic_bsz:
                # Dynamic seqlen balancing further splits a minibatch by token budget. If we shrink
                # the outer mini-batch down to 1 just to satisfy divisibility, a single long sample
                # can require multiple micro-batches and trip rearrange_micro_batches() assertions.
                effective = actual_batch_size
                logger.warning(
                    "update_actor: adjusting ppo_mini_batch_size %s -> %s for dynamic bsz "
                    "(actual_batch_size=%s)",
                    original_mini_batch_size,
                    effective,
                    actual_batch_size,
                )
            else:
                effective = original_mini_batch_size
                while effective > 1 and actual_batch_size % effective != 0:
                    effective -= 1
                logger.warning(
                    "update_actor: adjusting ppo_mini_batch_size %s -> %s (actual_batch_size=%s)",
                    original_mini_batch_size,
                    effective,
                    actual_batch_size,
                )
            self.actor.config.ppo_mini_batch_size = effective
        dataloader = self.actor.make_minibatch_iterator(data=data)
        self.actor.config.ppo_mini_batch_size = original_mini_batch_size
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def debug_materialize_optimizer_state(self):
        before_count = _count_megatron_optimizer_state_entries(self.actor_optimizer)
        log_with_rank(
            f"[MegatronWorker.debug_materialize_optimizer_state] enter role={self.role} "
            f"before_state_entries={before_count} offload_param={self._is_offload_param} "
            f"offload_optimizer={self._is_offload_optimizer}",
            rank=self.rank,
            logger=logger,
        )
        if self._is_offload_param:
        ...
        initialized = _initialize_megatron_optimizer_state_for_debug(self.actor_optimizer)
        after_count = _count_megatron_optimizer_state_entries(self.actor_optimizer)
        log_with_rank(
            f"[MegatronWorker.debug_materialize_optimizer_state] after_init role={self.role} "
            f"initialized_entries={initialized} after_state_entries={after_count}",
            rank=self.rank,
            logger=logger,
        )
        ...
        if self._is_offload_param:
            offload_megatron_model_to_cpu(self.actor_module)
        log_with_rank(
            f"[MegatronWorker.debug_materialize_optimizer_state] exit role={self.role} "
            f"after_state_entries={after_count}",
            rank=self.rank,
            logger=logger,
        )
    ...
    def save_checkpoint(self, checkpoint_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        log_with_rank(
            f"[SAVE_STAGE=WORKER_ENTER][MegatronWorker.save_checkpoint] role={self.role} path={checkpoint_path} "
            f"global_step={global_step} async_save={self.checkpoint_mananager.checkpoint_config.async_save} "
            f"offload_param={self._is_offload_param} offload_optimizer={self._is_offload_optimizer}",
            rank=self.rank,
            logger=logger,
        )
        if self._is_offload_param:
            log_with_rank(
                f"[SAVE_STAGE=WORKER_BEFORE_LOAD_MODEL_TO_GPU][MegatronWorker.save_checkpoint] path={checkpoint_path}",
                rank=self.rank,
                logger=logger,
            )
            load_megatron_model_to_gpu(self.actor_module)
            log_with_rank(
                f"[SAVE_STAGE=WORKER_AFTER_LOAD_MODEL_TO_GPU][MegatronWorker.save_checkpoint] path={checkpoint_path}",
                rank=self.rank,
                logger=logger,
            )
        if self.checkpoint_mananager.checkpoint_config.async_save and self._is_offload_optimizer:
            log_with_rank(
                f"[SAVE_STAGE=WORKER_BEFORE_LOAD_OPTIMIZER][MegatronWorker.save_checkpoint] path={checkpoint_path}",
                rank=self.rank,
                logger=logger,
            )
            load_megatron_optimizer(self.actor_optimizer)
            log_with_rank(
                f"[SAVE_STAGE=WORKER_AFTER_LOAD_OPTIMIZER][MegatronWorker.save_checkpoint] path={checkpoint_path}",
                rank=self.rank,
                logger=logger,
            )
        log_with_rank(
            f"[SAVE_STAGE=WORKER_BEFORE_MANAGER_SAVE][MegatronWorker.save_checkpoint] path={checkpoint_path}",
            rank=self.rank,
            logger=logger,
        )
        ...
        log_with_rank(
            f"[SAVE_STAGE=WORKER_AFTER_MANAGER_SAVE][MegatronWorker.save_checkpoint] path={checkpoint_path}",
            rank=self.rank,
            logger=logger,
        )
        log_with_rank(
            f"[SAVE_STAGE=WORKER_BEFORE_OUTER_BARRIER][MegatronWorker.save_checkpoint] path={checkpoint_path}",
            rank=self.rank,
            logger=logger,
        )
        torch.distributed.barrier()
        log_with_rank(
            f"[SAVE_STAGE=WORKER_AFTER_OUTER_BARRIER][MegatronWorker.save_checkpoint] path={checkpoint_path}",
            rank=self.rank,
            logger=logger,
        )
        if self._is_offload_param:
            offload_megatron_model_to_cpu(self.actor_module)
            log_with_rank(
                f"[SAVE_STAGE=WORKER_AFTER_OFFLOAD_MODEL_TO_CPU][MegatronWorker.save_checkpoint] path={checkpoint_path}",
                rank=self.rank,
                logger=logger,
            )
        if self.checkpoint_mananager.checkpoint_config.async_save and self._is_offload_optimizer:
            offload_megatron_optimizer(self.actor_optimizer)
            log_with_rank(
                f"[SAVE_STAGE=WORKER_AFTER_OFFLOAD_OPTIMIZER][MegatronWorker.save_checkpoint] path={checkpoint_path}",
                rank=self.rank,
                logger=logger,
            )
        log_with_rank(
            f"[SAVE_STAGE=WORKER_EXIT][MegatronWorker.save_checkpoint] role={self.role} path={checkpoint_path}",
            rank=self.rank,
            logger=logger,
        )
...
class CriticWorker(MegatronWorker, DistProfilerExtension):
    ...
    def _build_critic_model_optimizer(
        ...
        if self.config.load_weight:
            t0 = time.time()
            if self.config.megatron.use_dist_checkpointing and self.config.megatron.dist_checkpointing_path:
                load_mcore_dist_weights(
                    critic_module,
