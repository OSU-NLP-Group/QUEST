...
def _get_job_id_suffix() -> str:
    """Build a stable per-job suffix for named Ray actors."""
    try:
...
def _get_vllm_server_actor_name(replica_rank: int, node_rank: int, is_reward_model: bool = False) -> str:
    prefix = "vllm_server_reward" if is_reward_model else "vllm_server"
    return f"{prefix}_{_get_job_id_suffix()}_{replica_rank}_{node_rank}"
...
class vLLMReplica(RolloutReplica):
    ...
    async def launch_servers(self):
        ...
        for node_rank in range(nnodes):
            ...
            name = _get_vllm_server_actor_name(
                replica_rank=self.replica_rank,
                node_rank=node_rank,
                is_reward_model=self.is_reward_model,
            )
            # Trainer and rollouter may initialize async managers in the same Ray job.
            # Reuse an existing named server actor if present to avoid ActorAlreadyExistsError.
            try:
                server = ray.get_actor(name)
            except ValueError:
                try:
