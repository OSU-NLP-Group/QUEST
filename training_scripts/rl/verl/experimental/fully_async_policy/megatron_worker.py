...
class DetachNcclSync(BaseDetachNcclSync, AsyncActorRolloutRefWorker):
    ...
    def sync_rollout_weights(self, sync_group_name="actor_rollout"):
        ...
        if self._is_rollout and (not self._is_actor):
            if rollout_name == "vllm":
                inference_model = BaseDetachNcclSync.get_inference_model(self.rollout, self._run_async_safely)
                if not getattr(inference_model, "_verl_remote_weight_loader", False):
    ...
    def save_model_to_cpu(self, n):
        ...
        self.cpu_saved_models[n] = copy_megatron_model_to_cpu(self.actor_module)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def restore_model_from_cpu(self, n):
        if n in self.cpu_saved_models:
            restore_megatron_model_from_cpu(self.actor_module, self.cpu_saved_models[n])
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def has_cpu_model(self, n):
        return hasattr(self, "cpu_saved_models") and n in self.cpu_saved_models
