...
class MessageQueue:
    ...
    async def dump_state(self) -> dict[str, Any]:
        """Capture queue contents for checkpoint resume."""
        async with self._lock:
            return {
                "queue": list(self.queue),
                "val_queue": list(self.val_queue),
                "current_param_version": self.current_param_version,
                "total_produced": self.total_produced,
                "total_consumed": self.total_consumed,
                "dropped_samples": self.dropped_samples,
    ...
    async def load_state(self, state: dict[str, Any] | None):
        """Restore queue contents from a checkpoint snapshot."""
        if state is None:
            return
        async with self._lock:
            queue_items = list(state.get("queue", []))
            val_items = list(state.get("val_queue", []))
            max_queue_size = int(state.get("max_queue_size", self.max_queue_size))
            self.max_queue_size = max_queue_size
            self.queue = deque(queue_items, maxlen=max_queue_size)
            self.val_queue = deque(val_items)
...
class MessageQueueClient:
    ...
    def dump_state_sync(self) -> dict[str, Any]:
        """Capture queue contents for checkpoint resume."""
        return ray.get(self.queue_actor.dump_state.remote())

    def load_state_sync(self, state: dict[str, Any] | None):
        """Restore queue contents from a checkpoint snapshot."""
        return ray.get(self.queue_actor.load_state.remote(state))
