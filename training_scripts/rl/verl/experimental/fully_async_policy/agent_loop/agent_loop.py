...
class FullyAsyncAgentLoopWorker(AgentLoopWorker):
    ...
    async def generate_sequences_no_post(
        ...
        if not is_cancel:
            # In async reward mode, any non-cancelled output must already carry reward_score.
            if self.reward_loop_worker_handles is not None:
                missing_reward_idx = [i for i, output in enumerate(outputs) if output.reward_score is None]
                if missing_reward_idx:
                    raise RuntimeError(
                        "Missing reward_score for non-cancelled outputs in async reward mode. "
                        f"indices={missing_reward_idx}"
                    )
    ...
    def _addition_process(self, output: DataProto):
        ...
        # Use dtype=object to prevent numpy from inferring 2D shape when
        # inner lists happen to have equal lengths, which causes dimension
        # mismatch errors during DataProto.concat().
        pt_arr = np.empty(len(processing_times_list), dtype=object)
        pt_arr[:] = processing_times_list
        tc_arr = np.empty(len(tool_calls_times_list), dtype=object)
        tc_arr[:] = tool_calls_times_list
        output.non_tensor_batch["processing_times"] = pt_arr
        output.non_tensor_batch["tool_calls_times"] = tc_arr
        return output
    ...
    async def _partial_run_agent_loop(
        ...
        **kwargs,
        ...
        # Resuming from a previous partial state.
        prev_output = kwargs.get("output")
        if prev_output is not None:
            if prev_output.extra_fields.get("is_cancel", False):
                cancel_stage = prev_output.extra_fields.get("cancel_stage", "")
                if cancel_stage == "reward_pending":
                    logger.info("In _partial_run_agent_loop, resuming from reward_pending stage.")
                    prev_output.extra_fields["is_cancel"] = False
