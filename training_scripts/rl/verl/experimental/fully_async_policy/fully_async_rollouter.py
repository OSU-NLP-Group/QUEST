...
class FullyAsyncRollouter(SeparateRayPPOTrainer):
    ...
    def _rollouter_state_path(self, checkpoint_folder: str) -> str:
        return os.path.join(checkpoint_folder, "rollouter_state.pt")
    ...
    def __init__(
        ...
        if self.total_rollout_steps <= 0:
            raise ValueError(
                "[FullyAsyncRollouter] total_rollout_steps is 0, so no samples will be fed and the trainer will see "
                "'Collected 0/N samples'. Common causes: (1) train_dataloader is empty (len=0), e.g. all samples "
                "were filtered by max_prompt_length (data.max_prompt_length / filter_overlong_prompts), or train "
                "data path is wrong; (2) rollout.total_rollout_steps is set to 0. Check dataset size and "
                "data.max_prompt_length / data.train_files."
            )
        if self.config.rollout.total_epochs <= 0:
            raise ValueError(
                "[FullyAsyncRollouter] rollout.total_epochs must be >= 1 (got %s). With 0 epochs the feed iterator "
                "yields no batches and the trainer will see 0 samples."
    ...
    async def _wait_for_next_active_task(self) -> bool:
        async with self.lock:
            if not self.active_tasks:
                return False
            wait_tasks = tuple(self.active_tasks)
    ...
    async def _wait_for_all_active_tasks(self) -> None:
        while await self._wait_for_next_active_task():
            pass
    ...
    def get_replicas(self):
        """Get rollout replicas for checkpoint manager compatibility."""
    ...
    async def update_param_version(
        ...
        async with self.lock:
            ...
            if not need_validate:
                ...
                # Resume vLLM replicas before validation so that generate_for_partial
                # does not immediately return is_cancel=True (servers are paused after
                # cancel() in pause()).  Re-cancel afterwards to restore paused state.
            ...
            previous_validate_task = self.validate_task

        if need_validate and self.parallel_validate_and_rollout:
            if previous_validate_task is not None:
                if not previous_validate_task.done():
                    print("[FullyAsyncRollouter] validate_task is running, wait last validate_task to finish")
                await previous_validate_task
    ...
    async def save_checkpoint(self, local_global_step_folder: str):
        ...
        # Resume intentionally starts a fresh trigger cycle, so we do not persist
        # in-flight async queues or MQ payloads here.
        ...
        try:
            ...
            async with self.lock:
                rollouter_state = {
                    ...
                    "total_generated_samples": int(self.total_generated_samples),
                    "staleness_samples": int(self.staleness_samples),
                    "dropped_stale_samples": int(self.dropped_stale_samples),
                    "processed_sample_count": int(self.processed_sample_count),
                    "global_steps": int(self.global_steps),
                }

            rollouter_state_path = self._rollouter_state_path(local_global_step_folder)
            torch.save(rollouter_state, rollouter_state_path)
            print(f"[FullyAsyncRollouter] Saved rollouter state to {rollouter_state_path}")
        finally:
    ...
    def load_checkpoint(self):
        ...
        if self.config.trainer.resume_mode == "auto":
            if global_step_folder is None:
                print("[FullyAsyncRollouter] Training from scratch (no complete fully-async checkpoint found)")
                return 0
            ...
            assert self._checkpoint_name_rank(
                ...
                "[FullyAsyncRollouter] resume_from_path must be step_<n>, savefreq_step_<n>, or legacy global_step_<n>"
            )
            ...
            assert self._has_complete_async_checkpoint(global_step_folder), (
                f"[FullyAsyncRollouter] Incomplete fully-async checkpoint at {global_step_folder}"
            )
        else:
        ...
        trainer_global_steps = self._extract_checkpoint_step(global_step_folder)
        self.global_steps = (
        ...
        rollouter_state_path = self._rollouter_state_path(global_step_folder)
        if os.path.exists(rollouter_state_path):
            rollouter_state = torch.load(rollouter_state_path, weights_only=False)
            self.current_param_version = int(rollouter_state.get("current_param_version", self.current_param_version))
            self.total_generated_samples = int(
                rollouter_state.get("total_generated_samples", self.total_generated_samples)
            )
            # Resume starts a fresh trigger cycle, so no stale in-flight samples are carried over.
            self.staleness_samples = 0
            self.dropped_stale_samples = int(
                rollouter_state.get("dropped_stale_samples", self.dropped_stale_samples)
            )
            self.processed_sample_count = int(
                rollouter_state.get("processed_sample_count", self.processed_sample_count)
            )
            self.global_steps = int(rollouter_state.get("global_steps", self.global_steps))

            self.pending_queue = asyncio.Queue(maxsize=128)
            self.cancel_queue = asyncio.Queue()
            print(
                "[FullyAsyncRollouter] Restored rollouter state: "
                f"global_steps={self.global_steps}, current_param_version={self.current_param_version}, "
                "pending=0, cancel=0, staleness_samples=0"
            )
    ...
    async def _processor_worker(self):
        ...
        while True:
            ...
            while len(self.active_tasks) >= self.max_concurrent_samples:
                await self._wait_for_next_active_task()
    ...
    async def _process_single_sample_streaming(self, rollout_sample: RolloutSample):
        ...
        try:
            ...
            if not is_cancel:
                session_level_reward = self.config.actor_rollout_ref.rollout.multi_turn.get("session_level_reward", True)
                session_weight_correction = self.config.actor_rollout_ref.actor.get("session_weight_correction", False)
                ret = expand_multi_session_output(
                    ret,
                    tokenizer=self.tokenizer,
                    prompt_length=self.config.actor_rollout_ref.rollout.prompt_length,
                    response_length=self.config.actor_rollout_ref.rollout.response_length,
                    session_level_reward=session_level_reward,
                    session_weight_correction=session_weight_correction,
                )
                self._print_rollout_preview(ret)
            ...
            print(
                f"[FullyAsyncRollouter] Error processing sample {rollout_sample.sample_id}: {e}",
                flush=True,
            )
            raise
        finally:
    ...
    async def _streaming_generation_main(self):
        ...
        try:
            ...
            tasks_to_stop = []
            for task in (self.feed_task, self.processor_task):
                if task is None:
                    continue
                if not task.done():
                    task.cancel()
            ...
            if error is not None and self.config.async_training.partial_rollout:
                await self.async_rollout_manager.cancel()
            if active_tasks:
        ...
        # Send a finish signal only after a graceful completion.
        await self.message_queue_client.put_sample(sample=None, param_version=self.current_param_version)
