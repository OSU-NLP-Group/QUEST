...
class OneStepOffRayTrainer(SeparateRayPPOTrainer):
    ...
    @staticmethod
    def _align_batch_with_rollout_output(batch: DataProto, gen_batch_output: DataProto) -> DataProto:
        """Align repeated batch rows with rollout output rows.

        When DeepResearch session expansion is enabled, one rollout trajectory can be expanded
        into multiple session trajectories. In that case, rollout output carries `rollout_index`
        to map each expanded session back to the original repeated rollout row.
        """
        if len(batch) == len(gen_batch_output):
            return batch

        rollout_index = gen_batch_output.non_tensor_batch.get("rollout_index")
        if rollout_index is None:
            raise ValueError(
                "Rollout output batch size does not match repeated batch size, "
                "and `rollout_index` is missing for alignment."
            )

        if isinstance(rollout_index, np.ndarray):
            index_array = rollout_index.astype(np.int64, copy=False)
        else:
            index_array = np.array(rollout_index, dtype=np.int64)

        if index_array.ndim != 1:
            raise ValueError(f"`rollout_index` must be 1-D, got shape {index_array.shape}")
        if len(index_array) == 0:
            return batch[:0]
        if int(index_array.min()) < 0 or int(index_array.max()) >= len(batch):
            raise IndexError(
                f"`rollout_index` out of range: min={int(index_array.min())}, "
                f"max={int(index_array.max())}, batch_size={len(batch)}"
            )

        return batch[index_array.tolist()]
    ...
    async def _async_gen_next_batch(self, continuous_iterator):
        ...
        # For DeepResearch session expansion, align repeated batch with expanded rollout rows.
        batch = self._align_batch_with_rollout_output(batch, gen_batch_output)
    ...
    def _fit_update_weights(self):
        # Keep rollout policy one-step behind actor by synchronizing every step.
