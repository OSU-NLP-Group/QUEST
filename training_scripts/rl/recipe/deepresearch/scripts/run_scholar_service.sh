#!/usr/bin/env bash
# 在单独机器上启动 Scholar HTTP 服务（建议 2×A100：embedding 用 GPU，FAISS 用 CPU）
# 直接运行即可，默认会读同 repo 的 config/tools.yaml（含 faiss_similarity_threshold、serper 等）
# 覆盖配置：export SCHOLAR_SERVICE_CONFIG=/other/path/tools.yaml
# 覆盖端口：export SCHOLAR_SERVICE_PORT=8001
