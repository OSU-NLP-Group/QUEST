...
"""
HTTP service for DeepResearch scholar: SQLite cache + FAISS + Serper.
...
def _load_config() -> dict:
    """Load from env vars; override from SCHOLAR_SERVICE_CONFIG YAML if provided."""
    config = {
        "serper_key": os.getenv("SERPER_KEY_ID", ""),
        "topk": int(os.getenv("SCHOLAR_TOP_K", "10")),
        ...
        "cache_dir": os.getenv("SCHOLAR_CACHE_DIR", ""),
        "cache_file": os.getenv("SCHOLAR_CACHE_FILE", ""),
        ...
        "faiss_enabled": _bool_env("SCHOLAR_FAISS_ENABLED", True),
        "faiss_similarity_threshold": float(os.getenv("SCHOLAR_FAISS_SIMILARITY_THRESHOLD", "0.85")),
        "faiss_embedding_model": os.getenv("SCHOLAR_FAISS_EMBEDDING_MODEL", "sentence-transformers/all-MiniLM-L6-v2"),
        "faiss_device": os.getenv("SCHOLAR_FAISS_DEVICE", "cuda"),
        "max_workers": int(os.getenv("SCHOLAR_SERVICE_MAX_WORKERS", "1000")),
        "max_inflight_queries": int(os.getenv("SCHOLAR_SERVICE_MAX_INFLIGHT_QUERIES", "2000")),
        "serper_reserved_workers": int(os.getenv("SCHOLAR_SERVICE_SERPER_RESERVED_WORKERS", "32")),
        "queue_wait_timeout_sec": float(os.getenv("SCHOLAR_SERVICE_QUEUE_WAIT_TIMEOUT_SEC", "90")),
        "request_batch_size": int(os.getenv("SCHOLAR_SERVICE_REQUEST_BATCH_SIZE", "128")),
        "batch_window_sec": float(os.getenv("SCHOLAR_SERVICE_BATCH_WINDOW_SEC", "0.05")),
        "max_batch_queue_size": int(os.getenv("SCHOLAR_SERVICE_MAX_BATCH_QUEUE_SIZE", "0")),
        "faiss_write_queue_size": int(os.getenv("SCHOLAR_FAISS_WRITE_QUEUE_SIZE", "5000")),  # FAISS main queue capacity; when full, buffer in overflow
        "faiss_write_batch_size": int(os.getenv("SCHOLAR_FAISS_WRITE_BATCH_SIZE", "512")),  # FAISS batch write size (per worker; internally capped at 512)
        "faiss_write_flush_ms": int(os.getenv("SCHOLAR_FAISS_WRITE_FLUSH_MS", "30000")),  # Max wait time for FAISS batch writes (ms)
        "faiss_write_retry_max": int(os.getenv("SCHOLAR_FAISS_WRITE_RETRY_MAX", "-1")),  # -1 means unlimited retries
        "faiss_write_retry_backoff_ms": int(os.getenv("SCHOLAR_FAISS_WRITE_RETRY_BACKOFF_MS", "100")),
        "faiss_write_overflow_log_every": int(os.getenv("SCHOLAR_FAISS_WRITE_OVERFLOW_LOG_EVERY", "200")),
        "faiss_write_drain_timeout_sec": float(os.getenv("SCHOLAR_FAISS_WRITE_DRAIN_TIMEOUT_SEC", "120")),
        "faiss_read_gpus": os.getenv("SCHOLAR_FAISS_READ_GPUS", "0,1,2"),    # GPUs used by the read path (comma-separated)
        "faiss_read_threads_per_gpu": int(os.getenv("SCHOLAR_FAISS_READ_THREADS_PER_GPU", "16")),  # Number of read encoding threads per GPU
        "faiss_write_gpus": os.getenv("SCHOLAR_FAISS_WRITE_GPUS", "3"),  # GPUs used by the write path (comma-separated)
    }
    if not config["cache_dir"]:
        config["cache_dir"] = _DEFAULT_CACHE_DIR
    if not config["cache_file"]:
        config["cache_file"] = os.path.join(config["cache_dir"], "scholar.db")
    config_path = os.getenv("SCHOLAR_SERVICE_CONFIG")
    if config_path and os.path.exists(config_path):
        try:
            ...
            logger.warning("Failed to load SCHOLAR_SERVICE_CONFIG %s: %s", config_path, e)
    return config
...
class ScholarFaissTopKResponse(BaseModel):
    query: str = Field(..., description="Your input query")
    k: int = Field(..., description="Requested k")
    results: List[ScholarFaissTopKItem] = Field(default_factory=list, description="Top-k with similarity")


def create_scholar_app(config: Optional[dict] = None) -> FastAPI:
    ...
    class SerperClient:
        ...
        async def _search_async(self, query: str, topk: int) -> str:
                    ...
                    async with self.session.post(url, json=payload, headers=headers, timeout=self.timeout) as resp:
                        if resp.status >= 400:
                            ...
                            if resp.status >= 500 and attempt < self.max_retries:
                                ...
                                continue
                            return f"Google scholar failed: {resp.status} {text}"
            ...
            return "Google scholar failed: Timeout"
    ...
    def _enqueue_faiss_task(item: tuple[str, str, int]) -> None:
        ...
        if (
            ...
            logger.warning(
                "[scholar] FAISS write queue full, buffering overflow=%s (no drop)",
                overflow_size,
            )
    ...
    def _requeue_failed_faiss_item(query: str, result: str, retry: int) -> None:
        ...
        if _faiss_write_retry_max >= 0 and next_retry > _faiss_write_retry_max:
            ...
            logger.error(
                "[scholar] FAISS write dropped after retries=%s query=%r",
    ...
    def _flush_faiss_batch(
        ...
        try:
            ...
            logger.debug("[scholar] async cached (FAISS) worker=%s batch=%s", worker_id, n)
            with _faiss_write_seen_lock:
                for q in batch_queries:
                    _faiss_write_seen.discard(q)
        except Exception as e:
            logger.warning(
                "[scholar] async FAISS batch add failed worker=%s batch=%s: %s",
                worker_id, n, e,
            )
            for i in range(n):
    ...
    def _faiss_write_worker(worker_id: int, encoder) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.warning("[scholar] async FAISS add failed worker=%s query=%r: %s", worker_id, q_write[:80], e)
    ...
    if faiss_retriever is not None:
        ...
        try:
            ...
            if not _write_gpu_ids:
                ...
                for i, gpu_id in enumerate(_write_gpu_ids):
                    if gpu_id in _read_gpu_set and _read_encoders:
                        ...
                        logger.info("[scholar] write worker %s reuses read encoder on cuda:%s", i, gpu_id)
                    else:
                        device = f"cuda:{gpu_id}"
                        enc = SentenceTransformer(_embed_model, device=device)
                    _faiss_write_encoders.append(enc)
            ...
            logger.warning("[scholar] FAISS write init failed, no async write: %s", e)
    ...
    def search_one_with_source(query: str) -> tuple:
        ...
        if cache:
            ...
            if cached:
                logger.debug("[scholar] cache hit (exact) %s", q[:80])
        ...
        if faiss_retriever is not None:
            ...
            if faiss_result is not None:
                ...
                logger.debug("[scholar] cache hit (FAISS) %s score=%.3f", q[:80], score)
                return faiss_result, "faiss", round(score, 6), None
        result = google_scholar_sync(q, topk, serper_key)
    ...
    executor = ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="scholar")
    ...
    if max_inflight_queries > max_serper_inflight:
        logger.warning(
            "[scholar] reduce serper inflight %s -> %s to reserve %s worker threads",
    ...
    batch_processing_sem = asyncio.Semaphore(max(1, int(os.getenv("SCHOLAR_SERVICE_BATCH_CONCURRENCY", "64"))))
    ...
    async def _run_serper_with_gate(query: str):
        ...
        try:
            ...
            logger.warning(
                "[scholar] overloaded: serper gate wait > %.1fs query=%r",
        ...
        if waited > 1.0:
            logger.warning("[scholar] serper wait %.2fs query=%r", waited, q[:80])
        try:
            loop = asyncio.get_running_loop()
    ...
    async def _resolve_serper_for_future(query: str, fut: asyncio.Future) -> None:
        try:
            ...
            logger.warning("[scholar] batch serper failed query=%r: %s", (query or "")[:80], e)
            result = (
    ...
    async def _process_query_batch(items: list[tuple[str, asyncio.Future]]) -> None:
        ...
        if pending_queries and faiss_retriever is not None:
            ...
            try:
                ...
                logger.warning("[scholar] batch FAISS lookup failed (%s queries): %s", len(pending_queries), e)
                faiss_hits = [(None, 0.0)] * len(pending_queries)
        ...
        while not stop:
            ...
            async def _process_and_ack(items: list):
                try:
                    ...
                    logger.exception("[scholar] batch processing task failed")
    ...
    logger.info(
        "Scholar service pool: max_workers=%s serper_inflight=%s reserved_workers=%s queue_wait_timeout=%.1fs batch_size=%s batch_window=%.1fs batch_queue_max=%s",
    ...
    app = FastAPI(title="DeepResearch Scholar Service", version="0.1.0")
    ...
    async def startup():
        ...
        if batch_window_sec > 0:
            ...
            logger.info("[scholar] request batching enabled: window=%.1fs", batch_window_sec)
    ...
    async def shutdown():
        ...
        if _faiss_write_active:
            ...
            if pending > 0:
                logger.warning(
                    "[scholar] FAISS pending writes remain after %.1fs drain timeout: %s",
        ...
        for _ in _faiss_write_threads:
            try:
                ...
                logger.warning("[scholar] FAISS write queue full during shutdown, skipping sentinel")
    ...
    @app.post("/scholar", response_model=ScholarResponse)
    async def search(req: ScholarRequest):
        ...
        async def one(query: str):
            ...
            if batch_window_sec > 0:
                ...
                try:
                    ...
                    logger.warning("[scholar] overloaded: batch queue wait > %.1fs query=%r", queue_wait_timeout_sec, s[:80])
                    return (
                        "[Search Error] Search service overloaded (queue wait timeout), please retry.",
                        "serper",
                        None,
        ...
        sources = [ScholarSourceItem(source=t[1], similarity=t[2]) for t in out]
        combined = "\n=======\n".join(results)
        ...
        if elapsed > 60:
            logger.warning("[scholar] slow request: %.1fs for %s queries", elapsed, len(queries))
        else:
            logger.info("[scholar] request: %.1fs for %s queries", elapsed, len(queries))
        return ScholarResponse(text=combined, metrics=metrics, sources=sources)

    @app.get("/scholar_faiss_topk", response_model=ScholarFaissTopKResponse)
    async def scholar_faiss_topk(query: str = "", k: int = 10):
        ...
        items = [
            ScholarFaissTopKItem(rank=i + 1, cached_query=q, result=r, similarity=round(s, 6))
            for i, (q, r, s) in enumerate(rows)
        ]
        return ScholarFaissTopKResponse(query=query.strip(), k=k, results=items)

    logger.info("[startup] create_scholar_app total %.1fs", time.perf_counter() - _t0)
    return app


def main():
    import argparse
    parser = argparse.ArgumentParser(description="DeepResearch Scholar HTTP Service")
    parser.add_argument("--host", default="0.0.0.0", help="Bind host")
    parser.add_argument("--port", type=int, default=8000, help="Bind port")
    parser.add_argument("--config", default=None, help="Path to tools.yaml (overrides env)")
    args = parser.parse_args()
    if args.config:
        os.environ["SCHOLAR_SERVICE_CONFIG"] = args.config
    app = create_scholar_app()
    logger.info("Starting scholar service on %s:%s", args.host, args.port)
