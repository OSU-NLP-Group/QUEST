...
def rewrite_faiss_result_header(result_text: str, current_query: str) -> str:
    """FAISS 命中时把结果里的 'A Google scholar for ''...'' found N results' 改成当前 query，避免暴露原始 query。"""
...
def google_scholar_sync(query: str, topk: int = 10, serper_key: str = "") -> str:
    ...
    # 单次 Serper 调用超时，缩短到 30s 避免拖慢整次 /scholar（客户端 120s 超时）
...
class DeepResearchScholarTool(BaseTool):
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        # 若配置了 scholar_service_url / scholar_nodes_conf，则只做 HTTP 转发，不建本地 cache/FAISS
    ...
    def _scholar_via_service(self, queries: list[str]) -> tuple[str, dict]:
        """Sync: POST to scholar service, return (text, metrics). 超时/连接错误/5xx 时自动重试。"""
        if not requests:
