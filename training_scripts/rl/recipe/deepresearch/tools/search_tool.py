...
def _get_leader_flag(config: dict) -> bool:
    ...
    if os.getenv("SEARCH_CACHE_SHARD_LEADER") is not None:
        return _bool_env("SEARCH_CACHE_SHARD_LEADER", False)
...
class FaissRetriever:
    ...
    def __init__(
        ...
        self.index_path = index_path or os.path.join(cache_dir, "search_faiss.index")
        self.meta_path = meta_path or os.path.join(cache_dir, "search_faiss_meta.db")
    ...
    def _load(self) -> None:
        if os.path.exists(self.index_path) and os.path.exists(self.meta_path):
            try:
                ...
                logger.info("[search] FAISS loaded: %s vectors, next_id=%s", self._index.ntotal, self._next_id)
            except Exception as e:
                logger.warning("[search] FAISS load failed: %s, starting empty", e)
                self._index = None
    ...
    def search_top1_with_vec(self, vec: "np.ndarray") -> Tuple[Optional[str], float]:
        ...
        try:
            ...
            logger.warning("[search] FAISS meta read failed id=%s: %s", idx, e)
            return None, score
    ...
    def search_top1_with_vecs(self, vecs: "np.ndarray") -> list[Tuple[Optional[str], float]]:
        ...
        try:
            ...
            try:
                ...
                logger.warning("[search] FAISS batch meta read failed: %s", e)
                return out
    ...
    def save(self) -> None:
        ...
        try:
            if self._index is not None and self._index.ntotal > 0:
                ...
                logger.debug("[search] FAISS index saved to %s", self.index_path)
        finally:
            self._rwlock.write_release()
...
def merge_shards_into_master(
    ...
    for path in shard_files:
        ...
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute("SELECT query, result, timestamp FROM search_cache")
        rows = c.fetchall()
        shard_total += len(rows)
        for row in rows:
            cur.execute(
                """
                INSERT OR IGNORE INTO search_cache (query, result, timestamp)
                VALUES (?, ?, ?)
                """,
    ...
    logger.info(
        "[search] Merge: shards had %s rows, master %s -> %s (new: %s)",
        shard_total, before, after, new_rows,
    )
    return after


def build_faiss_from_search_cache(
    cache_dir: str,
    cache_file: str,
    embedding_model: str = "sentence-transformers/all-MiniLM-L6-v2",
    similarity_threshold: float = 0.85,
    ...
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    cur.execute("SELECT query, result FROM search_cache ORDER BY timestamp ASC")
    rows = cur.fetchall()
    ...
    n = len(rows)
    os.makedirs(cache_dir, exist_ok=True)
    ipath = index_path or os.path.join(cache_dir, "search_faiss.index")
    mpath = meta_path or os.path.join(cache_dir, "search_faiss_meta.db")
    ...
    if device == "cuda" and num_gpus is not None and num_gpus > 1:
        ...
        if ngpu <= 1:
            ...
            logger.info("[search] FAISS built from cache (multi-GPU): %s entries -> %s", n, ipath)
            return n
    ...
    faiss.write_index(index, ipath)
    meta_conn.close()
    logger.info("[search] FAISS built from cache: %s entries -> %s", len(rows), ipath)
    return len(rows)
...
def incremental_update_faiss(
    ...
    if not new_rows:
        meta_conn.close()
        logger.info("[search] FAISS incremental: no new entries")
        return 0
    ...
    if device == "cuda" and num_gpus is not None and num_gpus > 1:
        ...
        if ngpu > 1:
            ...
            if ngpu > 1:
                ...
                meta_conn.close()
                logger.info(
                    "[search] FAISS incremental (multi-GPU): added %s new entries (total now %s)",
                    added,
                    index.ntotal,
                )
                return added
    ...
    faiss.write_index(index, ipath)
    meta_conn.close()
    logger.info("[search] FAISS incremental: added %s new entries (total now %s)", added, index.ntotal)
    return added
...
class SearchCache:
    ...
    def _split_master(self) -> None:
        ...
        master_cursor.execute("SELECT query, result, timestamp FROM search_cache")
    ...
    def merge_shards(self) -> None:
        ...
        for shard_id, shard_file in enumerate(self._shard_files):
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT query, result, timestamp FROM search_cache")
...
def google_search_sync(query: str, topk: int = 10, serper_key: str = "") -> str:
    if not serper_key:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                return f"Google search Timeout after {max_retries + 1} attempts. Please try again later."
    ...
    try:
        ...
        for page in results.get("organic", [])[:topk]:
            ...
            formatted = (
                f"{idx}. [{page.get('title','')}]({page.get('link','')}){date_published}{source}\n{snippet}"
            )
...
class DeepResearchSearchTool(BaseTool):
    """Search tool with optional SQLite cache (sharded), or forward to HTTP search service."""
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        # Static fallback URL (used when search_nodes_conf is absent or fails to parse)
        self._search_service_url_static = (
            (config.get("search_service_url") or os.environ.get("SEARCH_SERVICE_URL") or "").rstrip("/")
        )
        self._use_search_service = bool(self._search_nodes_conf or self._search_service_url_static)
        if self._use_search_service:
            self.cache = None
            self.faiss_retriever = None
            self.serper_key = ""
            self.search_service_max_retries = int(config.get("search_service_max_retries") or os.environ.get("SEARCH_SERVICE_MAX_RETRIES", "3"))
            self.search_service_timeout = int(config.get("search_service_timeout") or os.environ.get("SEARCH_SERVICE_TIMEOUT", "120"))
            logger.info(
                "[search] Using HTTP search service (nodes_conf=%s, static_url=%s, max_retries=%s, timeout=%ss)",
                self._search_nodes_conf or "(none)",
                self._search_service_url_static or "(none)",
                self.search_service_max_retries,
                self.search_service_timeout,
            )
            return
        ...
        cache_dir = config.get("cache_dir") or os.getenv("SEARCH_CACHE_DIR", "")
        if not cache_dir:
            cache_dir = _DEFAULT_CACHE_DIR
        os.makedirs(cache_dir, exist_ok=True)

        cache_file = config.get("cache_file") or os.getenv("SEARCH_CACHE_FILE", "")
        if not cache_file:
            cache_file = os.path.join(cache_dir, "search_cache.db")
        ...
        if faiss_enabled is None:
            faiss_enabled = _bool_env("SEARCH_FAISS_ENABLED", False)
        if faiss_enabled and cache_enabled:
            faiss_threshold = config.get("faiss_similarity_threshold")
            if faiss_threshold is None:
                try:
                    faiss_threshold = float(os.getenv("SEARCH_FAISS_SIMILARITY_THRESHOLD", "0.85"))
                except ValueError:
                    faiss_threshold = 0.85
            faiss_model = config.get("faiss_embedding_model") or os.getenv("SEARCH_FAISS_EMBEDDING_MODEL", "sentence-transformers/all-MiniLM-L6-v2")
            try:
                self.faiss_retriever = FaissRetriever(
                    cache_dir=cache_dir,
                    embedding_model=faiss_model,
                    similarity_threshold=faiss_threshold,
                    index_path=config.get("faiss_index_path") or os.getenv("SEARCH_FAISS_INDEX_PATH"),
                    meta_path=config.get("faiss_meta_path") or os.getenv("SEARCH_FAISS_META_PATH"),
                )
                atexit.register(self._faiss_close)
                logger.info(
                    "[search] FAISS semantic retrieval enabled (threshold=%.2f, model=%s)",
                    self.faiss_retriever.threshold,
                    faiss_model,
                )
            except Exception as e:
                logger.warning("[search] FAISS retriever init failed: %s; using cache+Serper only", e)

        if not self.serper_key:
            logger.warning("SERPER_KEY_ID not set, search tool will not work")
    ...
    def _search_with_cache(self, query: str) -> Tuple[str, str, Optional[float]]:
        ...
        if self.cache:
            ...
            if cached:
                logger.info("[search] cache hit (exact) for query: %s", q)
        ...
        if self.faiss_retriever is not None:
            ...
            if faiss_result is not None:
                ...
                logger.info("[search] cache hit (FAISS) for query: %s (score=%.3f)", q, score)
                return faiss_result, "faiss", round(score, 6)

        # 3) Call real Serper API
        result = google_search_sync(q, self.topk, self.serper_key)
            ...
            if not _is_empty_search_result(result) and not _is_search_error_result(result):
                ...
                if self.faiss_retriever is not None:
                    try:
                        ...
                        logger.info("[search] cached (exact+faiss) for query=%r", q[:80])
                    except Exception as e:
                        logger.warning("[search] FAISS add failed: %s", e)
    ...
    def _get_search_service_url(self) -> str:
        """Hot-reload URL from search_nodes_conf on every call; fall back to static URL."""
        if self._search_nodes_conf:
            try:
                with open(self._search_nodes_conf, encoding="utf-8") as f:
                    for raw_line in f:
                ...
                logger.warning("[search] Failed to read search_nodes_conf %s: %s", self._search_nodes_conf, e)
    ...
    def _search_via_service(self, queries: list[str]) -> tuple[str, dict]:
        ...
        max_retries = self.search_service_max_retries
        timeout_sec = self.search_service_timeout
        last_error = None
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait = 2 ** attempt
                    logger.warning("[search] service timeout/connection (attempt %s/%s), retry in %ss: %s", attempt + 1, max_retries, wait, e)
                    time.sleep(wait)
                else:
                    logger.warning("[search] service call failed after %s retries: %s", max_retries, e)
                ...
                if e.response is not None and e.response.status_code >= 500 and attempt < max_retries - 1:
                    wait = 2 ** attempt
                    logger.warning("[search] service 5xx (attempt %s/%s), retry in %ss: %s", attempt + 1, max_retries, wait, e)
                    time.sleep(wait)
                else:
                    logger.warning("[search] service call failed: %s", e)
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        if not query:
            return ToolResponse(text="[Tool Error] Search query cannot be empty."), 0.0, {"error": "empty_query"}

        queries = query if isinstance(query, list) else [query]

        if self._use_search_service:
            loop = asyncio.get_running_loop()
            text, metrics = await loop.run_in_executor(None, self._search_via_service, queries)
        ...
        for q in queries:
            out.append(await loop.run_in_executor(None, self._search_with_cache, q))
