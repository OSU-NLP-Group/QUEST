...
class LLMClient():
    def __init__(self, provider, is_async=False):
        ...
        if provider == 'azure_openai':
            if is_async:
                from obj_task_eval.llm_client.azure_openai_client import AsyncAzureOpenAIClient
                self.client = AsyncAzureOpenAIClient()
            else:
                from obj_task_eval.llm_client.azure_openai_client import AzureOpenAIClient
            ...
            if is_async:
                from obj_task_eval.llm_client.local_openai_client import AsyncLocalOpenAIClient
                self.client = AsyncLocalOpenAIClient()
            else:
                from obj_task_eval.llm_client.local_openai_client import LocalOpenAIClient
                self.client = LocalOpenAIClient()

        elif provider == 'bedrock_anthropic':
            if is_async:
                from obj_task_eval.llm_client.bedrock_anthropic_client import AsyncBedrockAntrhopicClient
                self.client = AsyncBedrockAntrhopicClient()
            else:
                from obj_task_eval.llm_client.bedrock_anthropic_client import BedrockAntrhopicClient
