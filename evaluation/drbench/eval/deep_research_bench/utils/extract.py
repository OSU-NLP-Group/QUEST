...
def run(data, output_path, id_to_lang_map, model=None):
    for i, d in enumerate(data):
        ...
        # Use the specified model or fall back to the default from api.py
        if model:
            response = call_model(user_prompt, model=model)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", type=str, default=None, help="Model name in litellm format (e.g., 'gemini/gemini-2.5-flash-preview-05-20', 'gpt-4', 'claude-3-sonnet')")
    ...
    if n_total_process == 1:
        run(data_to_process, output_path, id_to_lang_map, model=args.model)
        ...
        run_partial = partial(run, output_path=output_path, id_to_lang_map=id_to_lang_map, model=args.model)
