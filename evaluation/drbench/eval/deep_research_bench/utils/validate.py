...
def validate(data, id_to_lang_map, model=None):
    url = data[0]
...
if __name__ == '__main__':
    ...
    for d in tqdm(data_to_process):
        ...
        if n_total_process == 1:
            results = [validate(citation, id_to_lang_map, model=args.model) for citation in citations]
        elif n_total_process > 1:
            run_partial = partial(validate, id_to_lang_map=id_to_lang_map, model=args.model)
