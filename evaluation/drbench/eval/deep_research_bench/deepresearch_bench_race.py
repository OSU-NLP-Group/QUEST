...
# Configure logging - change the default level from INFO to WARNING.
logging.basicConfig(level=logging.WARNING, format='%(asctime)s - %(levelname)s - %(message)s')

# Keep important information visible by setting this module's logger to INFO.
...
def main():
    ...
    parser.add_argument('--eval_model', type=str,
                       default=os.environ.get("EVAL_MODEL", "gemini/gemini-2.5-pro-preview-06-05"),
                       help='LLM model for evaluation (litellm format, e.g., "gemini/gemini-2.5-pro", "gpt-4", "claude-3-sonnet-20240229")')
    parser.add_argument('--clean_model', type=str,
                       default=os.environ.get("CLEAN_MODEL", "gemini/gemini-2.5-flash-preview-05-20"),
                       help='LLM model for article cleaning (litellm format)')
    ...
    logger.info(f"Using cleaning model: {clean_model}")
    ...
    # Initialize AI clients with specified models
    llm_client = AIClient(model=eval_model)
