    ...
    def __init__(self, provider, is_async=False):
        ...
        if provider == 'azure_openai':
            ...
            if is_async:
                ...
                self.client = AsyncOpenAIClient()   # current default path
            else:
                from mind2web2.llm_client.openai_client import OpenAIClient
                self.client = OpenAIClient()
        elif provider == 'local_openai':
            if is_async:
                from mind2web2.llm_client.local_openai_client import AsyncLocalOpenAIClient
                self.client = AsyncLocalOpenAIClient()
            else:
                from mind2web2.llm_client.local_openai_client import LocalOpenAIClient
